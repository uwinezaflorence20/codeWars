const Home = () =>{
return(
    <div className="">
        <h2 className="text-2xl font-medium mb-6">Welcome</h2>
        <p className="text-xl mb-4">Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde ullam temporibus libero ipsam enim sint eos possimus f
            ugiat necessitatibus accusamus! Deserunt cumque aut eos impedit magni evenie
            t ipsa excepturi ipsum!</p>
                    <p className="text-xl mb-4">Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde ullam temporibus libero ipsam enim sint eos possimus f
            ugiat necessitatibus accusamus! Deserunt cumque aut eos impedit magni evenie
            t ipsa excepturi ipsum!</p>
                    <p className="text-xl mb-4">Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde ullam temporibus libero ipsam enim sint eos possimus f
            ugiat necessitatibus accusamus! Deserunt cumque aut eos impedit magni evenie
            t ipsa excepturi ipsum!</p>
                    <p className="text-xl">Lorem ipsum dolor sit amet consectetur adipisicing elit. Unde ullam temporibus libero ipsam enim sint eos possimus f
            ugiat necessitatibus accusamus! Deserunt cumque aut eos impedit magni evenie
            t ipsa excepturi ipsum!</p>

    </div>
)
}
export default Home;